...
pub fn render_agent_pipeline_cards(
    ...
    for (idx, agent) in agents.iter().enumerate() {
        ...
        let ttft = agent_metrics
            .and_then(|m| m.ttft_ms)
            .map(|ms| format!("{}ms", ms))
        ...
        let lines = vec![
            ...
            Line::from(vec![Span::styled(
                format!("M: {}", agent.config.model),
                Style::default().fg(Color::DarkGray),
            )]),
            Line::from(vec![
                Span::styled("State: ", Style::default().fg(Color::Gray)),
                Span::styled(
                    format!("{} {}", status_icon, agent.status.as_str()),
                    Style::default()
                        .fg(status_color)
                        .add_modifier(Modifier::BOLD),
                ),
            ...
            Line::from(vec![Span::styled(
                format!("Tokens: {:<4} | TTFT: {:<5}", tokens, ttft),
                Style::default().fg(Color::Rgb(150, 150, 160)),
            )]),
            Line::from(vec![Span::styled(
                format!("Speed: {:>4.1} tok/s", tps),
                Style::default().fg(if tps > 0.0 {
                    Color::LightGreen
                } else {
                    Color::DarkGray
                }),
